...
import {
    ...
    MANUAL_BIAS_STEP, ORIENTATION, CROTCH_CONTACT_ACTS, STORAGE,
} from './lelo-config.js';
...
import { classifyMessage, calibrateCharacter } from './lelo-scene-classifier.js';
...
function arousalText(a) {
    const tiers = [
        ...
        [0.0, '{{user}} is composed, only just beginning to warm.'],
    ];
...
function updateLore() {
    ...
    try {
        c.setExtensionPrompt(AROUSAL_KEY, `[Player Arousal]\n${substitute(arousalText(sensors.arousal))}`, 1, 0);
...
function onAiMessage(id) {
    ...
    const { actType, pace } = classifyMessage(msg.mes || '');
    if (actType) {
        ...
        if (actType !== currentAct || pace !== currentPace) {
            currentAct = actType; currentPace = pace; liveManualBias = 0; actuator.clearManualBias();
        }
        applyActuation();
    } else if (currentAct) {
        currentAct = null; applyActuation();
    }
}
...
const api = {
    ...
    arm: (token) => { safety.arm(token || `consent-${Date.now()}`); notify('success', 'Session armed.', 'armed'); notify('info', 'To calibrate arousal: wear the device, reach full arousal, then press “Set Full Arousal Point”.', 'howcal'); },
