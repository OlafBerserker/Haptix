...
let secondary = null;           // optional 2nd output device { connector, actuator } (power-user multi-toy)
let secondaryMode = 'v2';       // detected command path for the 2nd device
let stateListeners = [];
let loreTimer = null;
const _notified = {};
const _notifyLogAt = {};
...
// Fan a motor action out to the primary AND any connected secondary device (multi-toy power users).
function eachActuator(fn) { if (actuator) fn(actuator); if (secondary?.actuator) fn(secondary.actuator); }
function applyActuation() {
    const lvl = computeIntensity(currentAct, currentPace);
    eachActuator((a) => a.setTarget(currentAct, lvl));
    scheduleLore();
}
...
// Robust contact detection: a 2nd quiet LLM pass answers "is a character's body on the user?" The prompt
// and reply parser are exported (pure) so a live-LLM test can exercise the exact production path.
const ACT_SET = new Set(['teasing', 'handjob', 'blowjob', 'titjob', 'footjob', 'vaginal', 'anal', 'climax']);
const PACE_SET = new Set(['caress', 'slow', 'steady', 'fast', 'frantic']);
const INVOL_SET = new Set(['incidental', 'impact']);

export function buildClassifierPrompt(text) {
    return `You are a strict physical-contact classifier for a haptic device. Read the ROLEPLAY MESSAGE and reply with ONE compact JSON object and NOTHING else — no prose, no markdown, no code fences.
...
Fields:
 "contact": true or false — true ONLY for real physical contact ON the user right now.
 "act": one of teasing,handjob,blowjob,titjob,footjob,vaginal,anal,climax,none — sexual act performed ON the user, else none.
 "pace": one of caress,slow,steady,fast,frantic.
 "involuntary": one of none,incidental,impact — brief NON-sexual touch: incidental=brush/bump, impact=hit/slap; else none.
If the user is reaching orgasm / finishing / cumming / spilling over the edge, set "act":"climax" even if narrated as their own sensation.
Example: {"contact":true,"act":"handjob","pace":"slow","involuntary":"none"}
ROLEPLAY MESSAGE:
"""${(text || '').slice(0, 1200)}"""
JSON:`;
}

// Tolerant extraction: strips code fences, scans for the FIRST balanced {...}, coerces every field to the
// allowed enum (defends against models that emit prose, markdown, trailing junk, or off-vocabulary values).
export function parseClassifierReply(raw) {
    if (raw == null) return null;
    let s = String(raw).replace(/```[a-zA-Z]*/g, '').replace(/```/g, '').trim();
    let obj = null;
    // find a balanced object (handles trailing prose after the JSON, and nested braces)
    const start = s.indexOf('{');
    if (start >= 0) {
        let depth = 0;
        for (let i = start; i < s.length; i++) {
            if (s[i] === '{') depth++;
            else if (s[i] === '}') { depth--; if (depth === 0) { const cand = s.slice(start, i + 1); try { obj = JSON.parse(cand); } catch { /* keep scanning */ } if (obj) break; } }
        }
    }
    if (!obj || typeof obj !== 'object') return null;
    const contactRaw = obj.contact === true || /^(true|yes|1)$/i.test(String(obj.contact));
    const actRaw = String(obj.act ?? '').toLowerCase().trim();
    const paceRaw = String(obj.pace ?? '').toLowerCase().trim();
    const involRaw = String(obj.involuntary ?? '').toLowerCase().trim();
    const act = ACT_SET.has(actRaw) ? actRaw : null;
    return {
        // A named sexual act ON the user IS contact — repairs model self-contradictions (contact:false +
        // act:blowjob) and ensures climax fires even when narrated as the user's own sensation.
        contact: contactRaw || act !== null,
        act,
        pace: PACE_SET.has(paceRaw) ? paceRaw : 'steady',
        involuntary: INVOL_SET.has(involRaw) ? involRaw : null,
    };
}
...
async function classifyViaLLM(text) {
    ...
    try { return parseClassifierReply(await c.generateQuietPrompt(buildClassifierPrompt(text), false, true)); }
    catch { return null; }
}
...
function applyClassification(c, charName, act, pace, involuntary) {
    if (act) {
        ...
        if (act !== currentAct || pace !== currentPace) {
            currentAct = act; currentPace = pace; liveManualBias = 0; eachActuator((a) => a.clearManualBias());
        }
        applyActuation();
        statusMeta(`${act} — ${pace}`);
        return;
    ...
    if (involuntary && BLIP[involuntary] && actuator.blip) {
        eachActuator((a) => a.blip(BLIP[involuntary].amp, BLIP[involuntary].ms));
...
function onSecondaryState(state) {
    stateListeners.forEach((fn) => { try { fn(state); } catch { /* ignore */ } });
    if (state === ConnState.READY) {
        secondaryMode = secondary?.connector.detectedMode || secondaryMode;
        secondary?.actuator.start();
        notify('success', '2nd device connected.', 'sec-connected');
    } else if (state === ConnState.DISCONNECTED || state === ConnState.IDLE) {
        secondary?.actuator.stop();
    }
}
...
export function initCore() {
    ...
    sensors = createSensors({
        ...
        onManualAdjust: (adj) => { liveManualBias = Math.max(-0.3, Math.min(0.3, liveManualBias + adj.dir * MANUAL_BIAS_STEP)); eachActuator((a) => a.setManualBias(liveManualBias)); },
...
const api = {
    ...
    connectSecondary: (opts) => {
        if (!secondary) {
            const sConn = createConnector({ onState: onSecondaryState, onNotify: () => safety.recordNotify(), onLog: (m) => console.log(m) });
            const sAct = createActuator({ connector: sConn, safety, getCommandMode: () => secondaryMode, getArousal: () => sensors?.arousal ?? 0, getPatternStyle: () => patternStyle, onLog: (m) => console.log(m) });
            secondary = { connector: sConn, actuator: sAct };
        }
        return secondary.connector.requestAndConnect(opts);
    },
    disconnectSecondary: () => { try { secondary?.actuator.estop(); secondary?.connector.disconnect(); } catch { /* ignore */ } secondary = null; },
    ...
    disarm: () => { safety.disarm(); eachActuator((a) => a.estop()); notify('info', 'Session ended.', 'ended'); },
    isArmed: () => safety?.isArmed?.() ?? false,
    estop: () => { eachActuator((a) => a.estop()); notify('warning', 'EMERGENCY STOP.', 'estop'); },
    testAct: (actType, pace) => { currentAct = actType; currentPace = pace || 'steady'; liveManualBias = 0; eachActuator((a) => a.clearManualBias()); if (safety.isArmed()) applyActuation(); notify('info', `Testing: ${actType} / ${pace || 'steady'}…`, 'testing'); },
    ...
    setSequence: (name) => { sequenceName = SEQUENCES.includes(name) ? name : 'off'; eachActuator((a) => a.setSequence(sequenceName === 'off' ? null : createSequence(sequenceName))); notify('info', sequenceName === 'off' ? 'Sequence off (scene-driven).' : `Sequence: ${sequenceName}.`, 'seq'); },
    ...
    getStatus: () => ({
        ...
        secondary: !!secondary,
        secondaryState: secondary?.connector?.state ?? null,
        fullArousalPoint,
