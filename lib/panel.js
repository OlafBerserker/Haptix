...
function buildLauncher() {
    ...
    const b = h('button', { id: 'haptix-launch', title: 'ST Reacher — open/close haptics panel', onclick: toggleHapticPanel }, '💗');
...
function buildPanel() {
    ...
    // multi-toy (power users): a 2nd output device that mirrors the scene (e.g. one anal + one penis toy)
    panel.append(h('div', { class: 'htx-row' }, [
        h('button', { id: 'htx-connect2', onclick: () => onConnectSecondary(false), title: 'Connect a 2nd toy that buzzes along with the same scene' }, '+ 2nd device'),
        h('button', { id: 'htx-disconnect2', onclick: () => bridge.disconnectSecondary() }, 'Drop 2nd'),
    ]));
    ...
    panel.append(h('button', { id: 'htx-pstyle', style: 'margin-top:6px;width:100%',
        title: 'Complex = rich act-specific patterns (default); Basic = simple flat vibration',
        onclick: onCyclePatternStyle }, 'Patterns: Complex'));

    panel.append(h('button', { id: 'htx-meta', style: 'margin-top:6px;width:100%',
        title: 'Show a "Haptix — act — pace" status toast each turn so you can verify detection (deactivatable)',
        onclick: onToggleMeta }, 'Status line: On'));
...
async function onConnectSecondary(any) {
    try { await bridge.connectSecondary({ any: any === true }); }   // user-gesture chain preserved
    catch (e) { console.error('[lelo] 2nd connect FAILED:', e); setBadge('error', (e && e.message ? e.message : 'failed').slice(0, 48)); }
}
...
function onToggleMeta() {
    bridge.setMeta(bridge.getStatus().metaStatus === false);
}
...
function poll() {
    ...
    const psBtn = document.getElementById('htx-pstyle');
    if (psBtn) psBtn.textContent = `Patterns: ${(s.patternStyle || 'complex') === 'basic' ? 'Basic' : 'Complex'}`;
    const metaBtn = document.getElementById('htx-meta');
    if (metaBtn) metaBtn.textContent = `Status line: ${s.metaStatus === false ? 'Off' : 'On'}`;
