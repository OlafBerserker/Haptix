...
export function createSensors(cb = {}) {
    ...
    function updateOrientation(z, mag) {
        // V3: "up" = z-axis dominant + positive (held pointing up). Small-scale values (~0-150).
        const dom = mag > 1 ? z / mag : 0;
        let next = orientation;
        if (orientation === 'natural' && z > ORIENTATION.UP_Z_ENTER && dom > ORIENTATION.Z_DOMINANCE) next = 'up';
        else if (orientation === 'up' && (z < ORIENTATION.UP_Z_EXIT || dom < ORIENTATION.Z_DOMINANCE)) next = 'natural';
        if (next !== orientation) {
            const t = now();
