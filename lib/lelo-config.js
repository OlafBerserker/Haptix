...
// A regex act/contact only counts if a USER/body token sits NEAR the keyword — kills atmospheric false
// positives like "traces a pattern in the air" or "circles you slowly". The LLM contact-classifier,
// when available, supersedes this regex path entirely.
export const CONTACT_TOKENS = Object.freeze([
    'your', ' you ', ' you,', ' you.', 'cock', 'dick', 'shaft', 'penis', 'balls', 'glans', 'manhood',
    'member', 'pussy', 'cunt', 'clit', 'vagina', 'ass', 'asshole', 'hole', 'crotch', 'groin', 'tip',
    'between your', 'inside you', 'onto you', 'around you',
    'cum', 'cumming', 'orgasm', 'climax',   // culmination words are intrinsically contact (never atmospheric)
]);
export const CONTACT_WINDOW = 45;   // chars each side of the act keyword to search for a contact token

export const PACE_KEYWORDS = Object.freeze({
    caress: ['gently', 'softly', 'languid', 'tender', 'feather', 'barely', 'lightly', 'tease', 'delicate'],
...
export const ORIENTATION = Object.freeze({
    // V3 accel is small-scale (~0-150, big-endian), NOT ±1024. "Up" = z-axis dominates positive.
    UP_Z_ENTER: 85,           // z above this AND z-dominant => 'up'
    UP_Z_EXIT: 50,            // hysteresis: must fall below this to leave 'up' (anti-flicker)
    Z_DOMINANCE: 0.55,        // z / magnitude must exceed this to count as "pointing up"
    DEBOUNCE_MS: 1500,        // orientation must hold this long before state change commits
    PHRASE: '{{user}} is holding their crotch',
});

// Loop E — broad catch-all for BRIEF, INVOLUNTARY contact (NOT voluntary sexual acts) -> a short blip.
//   incidental = accidental brush/bump/press; impact = a hit/slap/shove (sharper, for vests too).
export const INCIDENTAL_KEYWORDS = Object.freeze([
    'brush', 'brushes', 'brushed', 'bump', 'bumps', 'bumped', 'graze', 'grazes', 'grazed',
    'presses against', 'pressed against', 'rubs against', 'rubbed against', 'knock into', 'knocks into',
    'squeeze past', 'squeezes past', 'squeezed past', 'leans into', 'leaned into', 'against you',
    'collide', 'collides', 'collided', 'jostle', 'jostles', 'jostled', 'nudge', 'nudges', 'nudged', 'grinds past',
]);
export const IMPACT_KEYWORDS = Object.freeze([
    'hit', 'hits', 'slap', 'slaps', 'slapped', 'punch', 'punches', 'punched', 'kick', 'kicks', 'kicked',
    'shove', 'shoves', 'shoved', 'struck', 'strikes', 'slam', 'slams', 'slammed', 'smack', 'smacks', 'smacked',
    'yank', 'yanks', 'tackle', 'tackles', 'tackled', 'knee', 'headbutt', 'whip', 'whips', 'lash',
]);
export const BLIP = Object.freeze({
    incidental: { amp: 0.35, ms: 350 },   // soft, brief
    impact: { amp: 0.85, ms: 220 },       // sharp, brief
});
